...
std::string convert_single_line(std::string gmi_line)
{
  ...
  for (auto pair : tags_map) {
    if (std::regex_match(gmi_line, std::regex(pair.first)))
    {
      ...
      std::string text;
      ss >> text; // ïğîïóñòèòü ïåğâûé ñèìâîë
      getline(ss, text); // ïğî÷èòàòü äî êîíöà
      result = "<" + tag + ">" + text + "</" + tag + ">";
      return result;
    }
  }
...
int main() {
  ...
  generateSite(input_directory, output_directory); // TODO: ÓÄÀËÈÒÜ ÏÅĞÂÛÉ ÏĞÎÁÅË È ÏĞÎÁÅËÛ ÌÅÆÄÓ ÇÀÌÅÍÈÒÜ ÍÀ 1
}